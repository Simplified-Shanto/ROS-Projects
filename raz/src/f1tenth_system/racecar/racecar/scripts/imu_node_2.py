...
def imu_publisher():
    ...
    # Loop to publish the combined IMU message and the transform
    ...
    while not rospy.is_shutdown():
        ...
        # Add the header to the IMU message
        imu_msg.header = Header()
        imu_msg.header.stamp = rospy.Time.now()  # Current timestamp
        imu_msg.header.frame_id = 'base_imu_link'  # The frame of reference

        # Publish the IMU message
        imu_pub.publish(imu_msg)

        # Broadcast the transform from 'base_imu_link' to the 'world' frame
        br.sendTransform(
            (0.0, 0.0, 0.0),  # Position of the IMU (you can modify if necessary)
            (quaternion_data.x, quaternion_data.y, quaternion_data.z, quaternion_data.w),  # Orientation in quaternion
            rospy.Time.now(),
            'base_imu_link',  # Child frame
